...
export function ReferenceTableSection<T extends ReferenceRecord>({
  ...
  const titleKey = `references.sections.${sectionKey}.title`;
  const descriptionKey = `references.sections.${sectionKey}.description`;
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">{t(titleKey)}</h2>
            <Badge tone="outline">{data.length}</Badge>
          </div>
          <p className="max-w-2xl text-sm text-slate-500 dark:text-slate-400">{t(descriptionKey)}</p>
        </div>
      </div>
